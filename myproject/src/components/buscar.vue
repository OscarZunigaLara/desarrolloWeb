...
<body>
    ...
    <div ref="container">
        ...
        <input CLASS= "buttonCrearCuenta" type="submit" value="Buscar" v-on:click ="crearCatalogo('buscar')"/>
...
export default {
  ...
  mounted(){
   ...
   methods: {
    ...
    crearCatalogo: function(event, buscar) {
      this.GetCatalog(event, this.buscar)
...
//         console.log(this.$refs)
    ...
    GetCatalog: function (event, buscar) {
      const article ={"cadena" :buscar}
      //alert(article.cadena)
      //alert(article.pagina)
      //alert(article)
      axios.post("https://dqllp5rc6g.execute-api.us-east-1.amazonaws.com/buscarProductos/buscarproducto", article)
        .then 	(response => {
            //alert(response.data)
